...
helpers.validateUnlockCapabilities = function validateUnlockCapabilities (caps = {}) {
  ...
  if (_.isNil(unlockKey) || unlockKey === '') {
    throw new Error('A non-empty unlockKey value must be provided');
  }

  if ([PIN_UNLOCK, PIN_UNLOCK_KEY_EVENT, FINGERPRINT_UNLOCK].includes(unlockType)) {
    if (!/^[0-9]+$/.test(_.trim(unlockKey))) {
      throw new Error(`unlockKey value '${unlockKey}' must only consist of digits`);
    }
  } else if (unlockType === PATTERN_UNLOCK) {
    if (!/^[1-9]{2,9}$/.test(_.trim(unlockKey))) {
      throw new Error(`unlockKey value '${unlockKey}' must only include from two to nine digits in range 1..9`);
    }
    if (/([1-9]).*?\1/.test(_.trim(unlockKey))) {
      throw new Error(`unlockKey value '${unlockKey}' must define a valid pattern where repeats are not allowed`);
    }
    ...
    if (!/.{4,}/g.test(unlockKey)) {
      throw new Error(`The minimum allowed length of unlockKey value '${unlockKey}' is 4 characters`);
    }
  } else {
    throw new Error(`Invalid unlockType '${unlockType}'. ` +
