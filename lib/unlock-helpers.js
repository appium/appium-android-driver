...
helpers.patternUnlock = async function (adb, driver, capabilities) {
  ...
  */
  ...
  for (let i in keys) {
    ...
    logger.info("Key : " + keys[i] + " - Pos: " + JSON.stringify(keyPos));
    if (parseInt(i) === 0) {
      actions.push({action: 'press', options: {element: null, x: keyPos.x, y: keyPos.y}});
      lastPos = keyPos;
      // actions.push({action: 'moveTo', options: {element: el.ELEMENT, x: keyPos.x, y: keyPos.y}});
      continue;
    }
    let moveTo = {x: (keyPos.x - lastPos.x), y: (keyPos.x - lastPos.y)};
    logger.info("Move To : " + keys[i] + " - Pos: " + JSON.stringify(moveTo));
    //actions.push({action: 'moveTo', options: {element: null, x: moveTo.x, y: moveTo.y}});
    //actions.push({action: 'moveTo', options: {element: null, x: keyPos.x, y: keyPos.y}});
    lastPos = keyPos;
  }
  actions.push({action: 'moveTo', options: {element: null, x: 137, y: 137}});
  actions.push({action: 'moveTo', options: {element: null, x: 137, y: 137}});
  actions.push({action: 'moveTo', options: {element: null, x: -137, y: 0}});
  actions.push({action: 'moveTo', options: {element: null, x: -137, y: 0}});
  actions.push({action: 'moveTo', options: {element: null, x: 0, y: -137}});
  actions.push({action: 'moveTo', options: {element: null, x: 137, y: -137}});
  actions.push({action: 'moveTo', options: {element: null, x: 137, y: 0}});
  actions.push({action: 'moveTo', options: {element: null, x: 0, y: 137}});
