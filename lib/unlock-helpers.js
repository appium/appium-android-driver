...
helpers.stringKeyToArr = function (key) {
  return key.trim().replace(/\s+/, '').split(/\s*/);
};
...
helpers.getPatternActions = function (keys, initPos, piece) {
  let actions = [];
  let lastPos;
  // TODO - this not work if the pattern goes to not consecutives pin
  // For ex. Moving from pin 3 to pin 4
  // FIX THIS RIGHT AWAY BRUNO
  for (let [i, key] of _.pairs(keys)) {
