...
commands.activateApp = async function activateApp (appId) {
  ...
  try {
    ...
    // Some cases, starting an pacckage can fix with '--pct-syskeys', '0',
    // but the option causes another issue. https://github.com/appium/appium/issues/16941#issuecomment-1129555512
    // So as this error message, it would be nice to guide mobile:startActivity command as well.
    this.log.errorAndThrow(`Cannot activate '${appId}' with 'monkey' command. Please consider to use 'am start' command to start the activity if needed. ` +
