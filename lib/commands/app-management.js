...
/**
*  Check the currently installed version of the application
*   @param {string} appId
*   @throws {Error} if device is unreachable
*   @returns {string[]} the versions of applications installed under that package
*/
commands.findVersions = async function (appId) {
  let versionList = [];
  if (appId) {
    try {
      const packageInfo = await this.adb.shell(['dumpsys', 'package', appId]);
      packageInfo.match(/versionName=.*/gm).forEach( function (item) { versionList.push((item.split('='))[1])});
      return versionList;
    } catch (e) {
      log.warn(`could not find app package for ${appId}`);
      return versionList;
    }
  }
  return versionList;
