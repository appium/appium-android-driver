...
export async function ensureDeviceLocale(language, country, script) {
  try {
    ...
    const desiredLocaleAbbr = toLocaleAbbr({language, country, script});
    let errMsg = `Cannot set the device locale to '${desiredLocaleAbbr}'.`;
    /** @type {string[]} */
    let suggestions = [];
    try {
    ...
    if (!_.isEmpty(suggestions)) {
      errMsg += ` You may want to apply one the following locales instead: ${suggestions}`;
    }
