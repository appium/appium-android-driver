...
async function initDeviceStreamingProc (adb, deviceInfo, opts = {}) {
  ...
  let isStarted = false;
  const deviceStreamingLogger = logger.getLogger(
    `${SCREENRECORD_BINARY}@${_.truncate(deviceInfo.udid, {length: 8})}`);
...
async function initGstreamerPipeline (deviceStreamingProc, deviceInfo, opts = {}) {
  ...
  const gstreamerLogger = logger.getLogger(
    `gst@${_.truncate(deviceInfo.udid, {length: 8})}`);
