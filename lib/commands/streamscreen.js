...
function abbreviateUdid (udid) {
  return _.truncate(udid, {
    length: 8,
    omission: '',
  });
}
...
async function initDeviceStreamingProc (adb, deviceInfo, opts = {}) {
  ...
  let isStarted = false;
  const deviceStreamingLogger = logger.getLogger(`${SCREENRECORD_BINARY}@${abbreviateUdid(deviceInfo.udid)}`);
...
async function initGstreamerPipeline (deviceStreamingProc, deviceInfo, opts = {}) {
  ...
  const gstreamerLogger = logger.getLogger(`gst@${abbreviateUdid(deviceInfo.udid)}`);
