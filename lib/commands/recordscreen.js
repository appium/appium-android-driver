...
async function scheduleScreenRecord (adb, recordingProperties) {
  ...
  const fullCmd = [...adb.executable.defaultArgs, 'shell', ...cmd];
  const recordingProc = new SubProcess(adb.executable.path, fullCmd);
  ...
  log.debug(`Invoking '${fullCmd.join(' ')}'`);
...
async function terminateBackgroundScreenRecording (adb) {
...
commands.startRecordingScreen = async function (options = {}) {
  ...
  if (await terminateBackgroundScreenRecording(this.adb)) {
...
commands.stopRecordingScreen = async function (options = {}) {
  ...
  if (_.isEmpty(this._screenRecordingProperties)) {
    ...
    try {
      await terminateBackgroundScreenRecording(this.adb);
    } catch (ign) {}
    return '';
  }
