...
commands.startRecordingScreen = async function ( filePath, videoSize, timeLimit, bitRate ) {
  ...
  */
  ...
  data = this.adb.shell(cmd).catch((err) => {
    throw new Error(err);
  }); 
...
commands.stopRecordingScreen = async function ( ) {
  await this.adb.killProcessesByName('screenrecord').catch((err) => {
    throw new Error(err);
  }); 
};
