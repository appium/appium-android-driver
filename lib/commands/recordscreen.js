...
async function extractCurrentRecordingPath (adb, pids) {
  ...
  log.debug(`Got the following output from lsof: ${output}`);
...
commands.startRecordingScreen = async function (options = {}) {
  const {remotePath, user, pass, method, videoSize, timeLimit, bitRate, forceRestart} = options;
  let result = '';
  if (this.isEmulator()) {
  ...
  if (!_.isEmpty(pids)) {
    ...
    try {
      if (_.isEmpty(this._recentScreenRecordingPath) && !forceRestart) {
        this._recentScreenRecordingPath = await extractCurrentRecordingPath(this.adb, pids);
      }
      for (const pid of pids) {
        await this.adb.shell(['kill', '-2', pid]);
      }
    } catch (err) {
  ...
  // Try to return the existing screen record content
  if (!_.isEmpty(this._recentScreenRecordingPath)) {
    try {
      if (!forceRestart) {
        result = await uploadRecordedMedia(this.adb, this._recentScreenRecordingPath,
          remotePath, {user, pass, method});
      }
    } catch (err) {
