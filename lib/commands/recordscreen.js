...
async function scheduleScreenRecord (adb, recordingProperties) {
  ...
  if (util.hasValue(timeLimit)) {
    let timeLimitArg = parseInt(timeLimit, 10);
    if (!isNaN(timeLimitArg)) {
      cmd.push('--time-limit',
        `${timeLimitArg <= MAX_RECORDING_TIME_SEC ? timeLimitArg : MAX_RECORDING_TIME_SEC}`);
    }
  }
  ...
  const recordingProc = new SubProcess(adb.executable.path,
    [...adb.executable.defaultArgs, 'shell', ...cmd]);

  recordingProc.on('end', () => {
    if (recordingProperties.stopped || !util.hasValue(timeLimit)) {
      return;
    }
    const totalDuration = process.hrtime(startTimestamp)[0];
    log.debug(`The overall screen recording duration is ${totalDuration}s so far`);
    const timeLimitInt = parseInt(timeLimit, 10);
    if (isNaN(timeLimitInt) || timeLimitInt >= totalDuration) {
      log.debug('There is no need to start the next recording chunk');
      return;
    }

    log.debug(`Starting the next chunk of screen recording in order to achieve ${timeLimitInt}s duration`);
  ...
  await retryInterval(10, RETRY_PAUSE, async () => {
    if (!await adb.fileExists(pathOnDevice)) {
      throw new Error(`The remote file '${pathOnDevice}' does not exist`);
    }
  });

  if (_.isArray(recordingProperties.records)) {
...
async function mergeScreenRecords (mediaFiles) {
  ...
  log.debug(`Generated merging config '${configFile}' with the content:\n${configContent}`);
  ...
  log.info(`Initiating screen records merge with command '${FFMPEG_BINARY} ${args.join(' ')}'`);
...
commands.startRecordingScreen = async function (options = {}) {
  ...
  } else if (!_.isEmpty(this._screenRecordingProperties)) {
    if (this._screenRecordingProperties.recordingProcess && this._screenRecordingProperties.recordingProcess.isRunning) {
      try {
        await this._screenRecordingProperties.recordingProcess.stop('SIGTERM', PROCESS_SHUTDOWN_TIMEOUT_SEC * 1000);
  ...
  if (isNaN(timeout) || timeout > MAX_TIME_SEC || timeout <= 0) {
    throw new Error(`The timeLimit value must be in range (0, ${MAX_TIME_SEC}] seconds. ` +
...
commands.stopRecordingScreen = async function (options = {}) {
  ...
  if (!_.isEmpty(this._screenRecordingProperties)) {
    ...
    if (this._screenRecordingProperties.recordingProcess && this._screenRecordingProperties.recordingProcess.isRunning) {
      try {
        await this._screenRecordingProperties.recordingProcess.stop('SIGINT', PROCESS_SHUTDOWN_TIMEOUT_SEC * 1000);
        this._screenRecordingProperties.recordingProcess = null;
      } catch (e) {
        log.warn(`Unable to stop screen recording. Continuing anyway. Original error: ${e.message}`);
      }
    }
  }

  if (_.isEmpty(this._screenRecordingProperties)) {
    log.info(`Screen recording has not been started by Appium. There is nothing to retrieve`);
    return '';
  }
  ...
  try {
    ...
    if (localRecords.length > 1) {
      ...
      try {
        ...
        log.warn(`Cannot merge the recorded files. The most recent screen recording is going to be sent as the result. ` +
          `Original error: ${e.message}`);
      }
    }
    result = await uploadRecordedMedia(this.adb, resultFilePath, remotePath, {user, pass, method});
  } finally {
