...
commands.startRecordingScreen = async function ( filePath, videoSize, timeLimit, bitRate ) {
  ...
  let loop = true;
  let opts = {};
  let {udid, emPort} = await helpers.getDeviceInfoFromCaps(opts);
  
  opts.udid = udid;
  opts.emPort = emPort;

  let adb = await helpers.createADB(opts.javaVersion,
                                    opts.udid,
                                    opts.emPort,
                                    opts.adbPort,
                                    opts.suppressKillServer);

  data = await adb.getApiLevel();
