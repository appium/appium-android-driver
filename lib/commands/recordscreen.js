...
async function scheduleScreenRecord (adb, recordingProperties) {
  ...
  if (util.hasValue(recordingProperties.currentTimeLimit)) {
    ...
    if (!isNaN(currentTimeLimitInt)) {
      cmd.push('--time-limit', currentTimeLimitInt % MAX_RECORDING_TIME_SEC);
    }
  }
  ...
  recordingProc.on('end', () => {
    ...
    if (isNaN(timeLimitInt) || timeLimitInt >= currentDuration) {
    ...
    log.debug(`Starting the next ${recordingProperties.currentTimeLimit % MAX_RECORDING_TIME_SEC}s-chunk ` +
      `of screen recording in order to achieve ${timeLimitInt}s duration`);
