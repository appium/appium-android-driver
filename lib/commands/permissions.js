...
const PERMISSION_ACTION = Object.freeze({
  GRANT: 'grant',
  REVOKE: 'revoke',
});
...
async function changePermissionsViaPm (permissions, appPackage, action) {
  if (!_.values(PERMISSION_ACTION).includes(action)) {
  ...
  if (_.toLower(permissions) === ALL_PERMISSIONS_MAGIC) {
    ...
    if (action === PERMISSION_ACTION.GRANT) {
 ...
 * @property {string} action [grant|allow] One of `PERMISSION_ACTION` values if `target` is set to 'pm',
 * otherwise 'allow'
...
commands.mobileChangePermissions = async function mobileChangePermissions (opts = {}) {
  const {
    ...
    action = _.toLower(opts.target) === PERMISSION_TARGET.APPOPS ? APPOPS_ACTION.ALLOW : PERMISSION_ACTION.GRANT,
