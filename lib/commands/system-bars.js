 ...
 * @returns {string} The actual output of the downstream console command.
 */
commands.performStatusBarCommand = async function performStatusBarCommand (opts = {}) {
  const {command} = requireArgs('command', opts);

  const invokeStatusBarCommand = async (cmd, ...args) => await this.adb.shell([
    'cmd', 'statusbar', cmd, ...args
  ]);
  const statusBarCommands = {
    expandNotifications: async () => await invokeStatusBarCommand('expand-notifications'),
    expandSettings: async () => await invokeStatusBarCommand('expand-settings'),
    collapse: async () => await invokeStatusBarCommand('collapse'),
    addTile: async () => await invokeStatusBarCommand(
      'add-tile', requireArgs('component', opts).component
    ),
    removeTile: async () => await invokeStatusBarCommand(
      'remove-tile', requireArgs('component', opts).component
    ),
    clickTile: async () => await invokeStatusBarCommand(
      'click-tile', requireArgs('component', opts).component
    ),
    getStatusIcons: async () => await invokeStatusBarCommand('get-status-icons'),
  };
