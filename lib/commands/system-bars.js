...
commands.mobilePerformStatusBarCommand = async function mobilePerformStatusBarCommand (opts = {}) {
  ...
  const toStatusBarCommandCallable = (cmd, argBuilder = null) => async () => await this.adb.shell([
    'cmd', 'statusbar', cmd, ...(argBuilder ? argBuilder() : [])
  ]);
  const statusBarCommands = _.fromPairs([
    ['expandNotifications', ['expand-notifications']],
    ['expandSettings', ['expand-settings']],
    ['collapse', ['collapse']],
    ['addTile', ['add-tile', () => requireArgs('component', opts).component]],
    ['removeTile', ['remove-tile', () => requireArgs('component', opts).component]],
    ['clickTile', ['click-tile', () => requireArgs('component', opts).component]],
