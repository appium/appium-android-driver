...
commands.performStatusBarCommand = async function performStatusBarCommand (opts = {}) {
  ...
  const commonCmdPrefix = ['cmd', 'statusbar'];
  const statusBarCommands = {
    expandNotifications: await this.adb.shell([...commonCmdPrefix, 'expand-notifications']),
    expandSettings: async () => await this.adb.shell([...commonCmdPrefix, 'expand-settings']),
    collapse: async () => await this.adb.shell([...commonCmdPrefix, 'collapse']),
    addTile: async () => {
      const {component} = requireArgs('component', opts);
      return await this.adb.shell([...commonCmdPrefix, 'add-tile', component]);
    },
    removeTile: async () => {
      const {component} = requireArgs('component', opts);
      return await this.adb.shell([...commonCmdPrefix, 'remove-tile', component]);
    },
    clickTile: async () => {
      const {component} = requireArgs('component', opts);
      return await this.adb.shell([...commonCmdPrefix, 'click-tile', component]);
    },
    getStatusIcons: async () => await this.adb.shell([...commonCmdPrefix, 'get-status-icons']),
  };
