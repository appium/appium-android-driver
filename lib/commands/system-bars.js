...
function parseWindows (lines) {
  ...
  let windowTitleIndent = null;
  for (const line of lines.split('\n').map(_.trimEnd)) {
    const currentIndent = line.length - _.trimStart(line).length;
    if (_.isNil(windowTitleIndent) || currentIndent <= windowTitleIndent) {
      const match = WINDOW_TITLE_PATTERN.exec(line);
      if (!match) {
        currentWindowTitle = null;
        continue;
      }
      currentWindowTitle = match[1];
      if (_.isNil(windowTitleIndent)) {
        windowTitleIndent = currentIndent;
      }
      continue;
    }
    if (!currentWindowTitle || currentIndent <= windowTitleIndent) {
      continue;
    }
    if (!_.isArray(windows[currentWindowTitle])) {
      windows[currentWindowTitle] = [];
    }
    windows[currentWindowTitle].push(line);
  }
  ...
  for (const [windowName, props] of _.toPairs(windows)) {
    if (windowName.startsWith(STATUS_BAR_WINDOW_NAME_PREFIX)) {
      result.statusBar = parseWindowProperties(windowName, props);
    } else if (windowName.startsWith(NAVIGATION_BAR_WINDOW_NAME_PREFIX)) {
      result.navigationBar = parseWindowProperties(windowName, props);
    }
  }
  for (const [window, namePrefix] of [
    ['statusBar', STATUS_BAR_WINDOW_NAME_PREFIX],
    ['navigationBar', NAVIGATION_BAR_WINDOW_NAME_PREFIX]
  ]) {
    if (_.isNil(result[window])) {
...
commands.getSystemBars = async function getSystemBars () {
  const out = await this.adb.shell(['dumpsys', 'window', 'windows']);
  if (!out) {
