...
commands.pullFile = async function (remotePath) {
  ...
  if (remotePath.startsWith(CONTAINER_PATH_MARKER)) {
    ...
    const packageId = remotePath.substring(CONTAINER_PATH_MARKER.length, remotePath.indexOf('/'));
    const pathInContainer = path.posix.resolve(`/data/data/${packageId}`, remotePath.substring(remotePath.indexOf('/') + 1));
    log.info(`Parsed package identifier '${packageId}' from '${remotePath}'. Will get the data from '${pathInContainer}'`);
    tmpDestination = `/data/tmp/${path.posix.basename(pathInContainer)}`;
    await this.adb.shell(
      [`run-as '${packageId.replace(/'/g, '\\\'')}' cat '${pathInContainer.replace(/'/g, '\\\'')}'>'${tmpDestination.replace(/'/g, '\\\'')}'`]);
  }
...
commands.pushFile = async function (remotePath, base64Data) {
  ...
  try {
    ...
    if (remotePath.startsWith(CONTAINER_PATH_MARKER)) {
      ...
      await this.adb.shell(
        [`run-as '${packageId.replace(/'/g, '\\\'')}' cat '${tmpDestination.replace(/'/g, '\\\'')}'>'${pathInContainer.replace(/'/g, '\\\'')}'`]);
    } else {
