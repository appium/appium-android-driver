...
commands.startActivity = async function (appPackage, appActivity,
                                         ...
                                         dontStopAppOnReset) {
  log.debug(`Starting pacakge '${appPackage}' and activity '${appActivity}'`);
  await this.adb.startApp({
    pkg: appPackage,
    activity: appActivity,
    waitPkg: appWaitPackage || appPackage,
    waitActivity: appWaitActivity || appActivity,
    action: intentAction || this.opts.intentAction,
    category: intentCategory || this.opts.intentCategory,
    flags: intentFlags || this.opts.intentFlags,
    optionalIntentArguments: optionalIntentArguments || this.opts.optionalIntentArguments,
    stopApp: !_.isUndefined(dontStopAppOnReset) ? !dontStopAppOnReset : (this.opts.stopAppOnReset || !this.opts.dontStopAppOnReset)
  });
};
