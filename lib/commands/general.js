...
/**
 * Get device time
 * @param {String} format A optional format for `adb shell date`. You can see help with `adb shell date --help`.
 *                        `iso-8601`format by default like `2018-06-09T16:21:54+0900`.
 *                        e.g.: `'+%Y-%m-%dT%T%z'`
 ...
 */
commands.getDeviceTime = async function (format) {
  log.info('Attempting to capture android device date and time');
  try {
    let formatString = '+%Y-%m-%dT%T%z';
    if (_.isString(format)) {
      formatString = format;
    }
    return (await this.adb.shell(['date', formatString])).trim();
  } catch (err) {
