...
async function getChromedriverPort (opts) {
  ...
  // which could look like [8000, 8001, [9000, 9500]], where each element
  // in the array is either a single port or an array with two elements
  ...
  if (foundPort === null) {
    throw new Error(`Could not find a free port for chromedriver using ` +
                    `chromedriverPorts spec ${JSON.stringify(opts.chromedriverPorts)}`);
  }
...
helpers.setupNewChromedriver = async function setupNewChromedriver (opts, curDeviceId, adb) {
  ...
  if (opts.chromedriverPort) {
    ...
    opts.chromedriverPort = await getChromedriverPort(opts);
  }
