...
import { util, zip, tempDir } from 'appium-support';
import { errors, downloadApp } from 'appium-base-driver';
import {
...
helpers.setupNewChromedriver = async function setupNewChromedriver (opts, curDeviceId, adb) {
  ...
  if (opts.chromedriverExecutableUrl) {
    if (!this.relaxedSecurityEnabled) {
      log.errorAndThrow(`Appium server must have relaxed security flag set in order to download the zip`);
    } else {
      if (chromedriverUrls[opts.chromedriverExecutableUrl]) {
        log.info(`Will reuse ChromeDriver downloaded from '${opts.chromedriverExecutableUrl}'`);
      } else {
        const tempDownloadDir = await tempDir.openDir();
        log.info(`Will download ChromeDriver zip from '${opts.chromedriverExecutableUrl}'`);
        const zipFile = await downloadApp(opts.chromedriverExecutableUrl, path.join(tempDownloadDir, 'chromedriver.zip'));
        await zip.extractAllTo(zipFile, tempDownloadDir);
        chromedriverUrls[opts.chromedriverExecutableUrl] = tempDownloadDir;
      }
    }
    opts.chromedriverExecutableDir = chromedriverUrls[opts.chromedriverExecutableUrl];
  }
