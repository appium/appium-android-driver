...
commands.setNetworkConnection = async function (type) {
  ...
  if (!airplaneMode) {
    await this.wrapBootstrapDisconnect(async () => {
      ...
      await this.adb.setDataState(data, !_.isUndefined(this.opts.avd));
    });
  }
...
commands.setWifiState = async function(wifi) {
  await this.adb.setWifiState(wifi, !_.isUndefined(this.opts.avd));
};
...
commands.toggleData = async function () {
  ...
  await this.wrapBootstrapDisconnect(async () => {
    await this.adb.setWifiAndData({data}, !_.isUndefined(this.opts.avd));
  });
};
...
commands.toggleWiFi = async function () {
  ...
  await this.wrapBootstrapDisconnect(async () => {
    await this.adb.setWifiAndData({wifi}, !_.isUndefined(this.opts.avd));
  });
};
...
commands.setGeoLocation = async function (location) {
  return await this.adb.setGeoLocation(location, !_.isUndefined(this.opts.avd));
};
