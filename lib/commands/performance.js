...
commands.getPerformanceData = async function (applicationPackageName, performanceDataType, performanceDataReadTimeout) {

  if(performanceDataType === "batteryinfo"){
    return this.getBatteryInfo(performanceDataReadTimeout);
  }
  else if(performanceDataType === "cpuinfo"){
    return this.getCPUInfo(applicationPackageName, performanceDataReadTimeout);
  }
  else if(performanceDataType === "memoryinfo"){
    return this.getMemoryInfo(applicationPackageName, performanceDataReadTimeout);
  }
  else if(performanceDataType === "networkinfo"){
    return this.getNetworkTrafficInfo(performanceDataReadTimeout);
  }
  else {
    throw new Error('No performance data of type \'' + performanceDataType + '\' found.');
  }

};

helpers.getCPUInfo = async function (applicationPackageName, performanceDataReadTimeout) {
    let returnValue = [];
    ...
    let tryCount = 0;
    cmd = ['dumpsys', 'cpuinfo', '|', 'grep', "\'" + applicationPackageName + "\'"];
    
    while(1){
      
      if(tryCount === (performanceDataReadTimeout + 1) ){
            throw new Error('No response received after ' + performanceDataReadTimeout);
        }
        else {
            ++ tryCount;
...
helpers.getBatteryInfo = async function (performanceDataReadTimeout) {
    let returnValue = [];
    returnValue[0] = ["power"];
...
helpers.getMemoryInfo = async function (applicationPackageName, performanceDataReadTimeout) {
    let returnValue = [];
    ...
    cmd = ['dumpsys', 'meminfo', "\'" + applicationPackageName + "\'", '|', 'grep', '-E', "\'Native|Dalvik|EGL|GL|TOTAL\'"];
...
helpers.getNetworkTrafficInfo = async function (performanceDataReadTimeout) {
    let returnValue = [];
  ...
  let tryCount = 0;
    ...
    while(1){
      ...
      data = await this.adb.shell(cmd);        
        ...
        if(data !== "" && data !== undefined && data !== null){
            let netVariables = [["bucketStart", "activeTime", "rxBytes", "rxPackets", "txBytes", "txPackets", "operations"], ["st", "activeTime", "rb", "rp", "tb", "tp", "op"]];
            let index = 0;
              ...
              if(arrayList.length > 0){
                ...
                for(let i=0 ; i < arrayList.length ; i++){
                  ...
                  if(start >= 0 ){
                    ...
                    returnValue[returnIndex ++] = [bucketStart, activeTime, rxBytes, rxPackets, txBytes, txPackets, operations];
