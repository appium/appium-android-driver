...
const SUPPORTED_PERFORMANCE_DATA_TYPES = {
  ...
  uiinfo: 'the ui performance statistics - frames of animation that are occurring during the recording phase'
};
...
commands.getPerformanceData = async function getPerformanceData (packageName, dataType, dataReadTimeout = 2) {
  ...
  switch (dataType) {
    case 'batteryinfo':
      ...
      break;
    case 'uiinfo':
      data = await this.getUIInfo(dataReadTimeout);
      break;
    case 'cpuinfo':
...
helpers.getUIInfo = async function getUIInfo (dataReadTimeout = 2) {
  return await retryInterval(dataReadTimeout, RETRY_PAUSE, async () => {
    let cmd = ['dumpsys', 'gfxinfo', `'${packageName}'`];
    let data = await this.adb.shell(cmd);
    if (!data) throw new Error('No data from dumpsys'); //eslint-disable-line curly
