...
const PerformanceMixin = {
  ...
  async getPerformanceData(packageName, dataType, retries = 2) {
    ...
    switch (_.toLower(dataType)) {
      case 'batteryinfo':
        result = await getBatteryInfo.bind(this)(retries);
        break;
      case 'cpuinfo':
        result = await getCPUInfo.bind(this)(packageName, retries);
        break;
      case 'memoryinfo':
        result = await getMemoryInfo.bind(this)(packageName, retries);
        break;
      case 'networkinfo':
        result = await getNetworkTrafficInfo.bind(this)(retries);
        break;
      default:
        throw new Error(
