...
helpers.getCPUInfo = async function (packageName, dataReadTimeout = 2) {
  ...
  return await retryInterval(dataReadTimeout, RETRY_PAUSE, async () => {
    let cmd = ['dumpsys', 'cpuinfo', '|', 'grep', `'${packageName}'`];
    ...
    let match = /(\d+)% user \+ (\d+)% kernel/.exec(data);
    if (!match) throw new Error(`Unable to parse cpu data: '${data}'`); //eslint-disable-line curly

    let user = match[1];
    let kernel = match[2];
    return [_.clone(CPU_KEYS), [user, kernel]];
  });
};
...
helpers.getMemoryInfo = async function (packageName, dataReadTimeout = 2) {
  return await retryInterval(dataReadTimeout, RETRY_PAUSE, async () => {
    ...
    let apilevel = parseInt(await this.adb.getApiLevel(), 10);
...
export { commands, helpers, SUPPORTED_PERFORMANCE_DATA_TYPES, CPU_KEYS,
         MEMORY_KEYS, BATTERY_KEYS, NETWORK_KEYS };
