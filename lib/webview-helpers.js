 ...
 *
 * @param {ADB} adb The ADB insatnce
 * @param {string} socketName Devtools Unix socket name
 * @param {?string|number} webviewDevtoolsPort The local port number or null to apply
 * autodetection
 * @param {!Function} collectorFunc Async function, which accepts single localPort parameter,
 * which is the local port number, that has been successfully allocated for devtools
 * communication
 * @returns {*} The result of the collectorFunc
 * @throws {Error} If local port allocation fails or `collectorFunc` throws an error
 */
async function collectWebviewDetails (adb, socketName, webviewDevtoolsPort, collectorFunc) {
  const localPort = await allocateDevtoolsPort(adb, socketName, webviewDevtoolsPort);
  try {
    return await collectorFunc(localPort);
  } finally {
...
helpers.getWebviews = async function getWebviews (adb, {
  ...
  if (ensureWebviewsHavePages || enableWebviewDetailsCollection) {
    ...
    for (const item of webviewsMapping) {
      detailCollectors.push((async () => {
        try {
          const collectorFunc = async (localPort) => {
            const webViewData = {};
            if (enableWebviewDetailsCollection) {
              webViewData.info = await cdpInfo(localPort);
            }
            if (ensureWebviewsHavePages) {
              webViewData.pages = await cdpList(localPort);
            }
            return webViewData;
          };
          item.devtoolsData = await collectWebviewDetails(adb, item.proc, webviewDevtoolsPort, collectorFunc);
        } catch (e) {
          logger.debug(e);
        }
      })());
  ...
  const cacheWebviewData = (wvName, data) => {
    if (data?.info) {
      WEBVIEWS_DETAILS_CACHE.set(wvName, { info: data.info });
  ...
  if (androidDeviceSocket) {
    for (const {webview, devtoolsData} of webviewsMapping) {
      if (ensureWebviewsHavePages && devtoolsData?.pages && !devtoolsData.pages.length) {
        continue;
      }

      result.push(webview);
      cacheWebviewData(webview, devtoolsData);
    }
  } else {
    for (const {webview, devtoolsData} of webviewsMapping) {
      try {
        ...
        const pkgMatch = WEBVIEW_PKG_REGEXP.exec(webview);
        const pkg = pkgMatch ? pkgMatch[1] : await helpers.procFromWebview(adb, webview);
        const wvName = `${WEBVIEW_BASE}${pkg}`;
        cacheWebviewData(wvName, devtoolsData);
        result.push(wvName);
      } catch (e) {
