...
const UNIX_SOCKET_LINE_PATTERN = /^(.+@[\w.]+.*)$/gm;
...
async function getPotentialWebviewProcs (adb) {
  ...
  if (_.isEmpty(names)) {
    logger.debug('Found no active devtools sockets. Other active sockets are:');
    let lineMatch;
    while ((lineMatch = UNIX_SOCKET_LINE_PATTERN.exec(out))) {
      logger.debug(lineMatch[1]);
    }
  } else {
...
helpers.getWebviews = async function getWebviews (adb, {
  ...
  const cacheWebviewDetails = (wvName, info) => {
    if (info) {
  ...
  if (androidDeviceSocket) {
    for (const {webview, info, pages} of webviewsMapping) {
      if (ensureWebviewsHavePages && pages?.length > 0) {
        continue;
      }

      result.push(webview);
      cacheWebviewDetails(webview, info);
    }
  } else {
    for (const {webview, pages, info} of webviewsMapping) {
      try {
        if (ensureWebviewsHavePages && pages?.length > 0) {
          continue;
        }
        ...
        result.push(wvName);
        cacheWebviewDetails(wvName, info);
      } catch (e) {
