...
class AndroidDriver extends BaseDriver {
  ...
  async startAndroidSession () {
    ...
    if (this.opts.disableWindowAnimation) {
      log.info('Animations are already disabled');
      if (await this.adb.isAnimationOn()) {
        log.info('Disabling animations as it is requested by noAnimation capability');
        await this.adb.setAnimationState(false);
        this._wasAnimationDisabled = true;
      }
    }
  ...
  async deleteSession () {
    ...
    if (this._wasAnimationDisabled) {
      log.info('Restoring the animation state');
