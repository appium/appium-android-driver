...
class AndroidDriver extends BaseDriver {
  ...
  async initAUT () {
    ...
    // Uninstall any apps
    if (this.opts.uninstallPackages) {
      let uninstallPackages;
      try {
        uninstallPackages = helpers.parseArray(this.opts.uninstallPackages);
      } catch (e) {
        log.errorAndThrow(`Could not parse "uninstallPackages" capability: ${e.message}`);
      }
      await helpers.uninstallPackages(uninstallPackages, this.adb);
    }
