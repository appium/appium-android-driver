...
class AndroidDriver extends BaseDriver {
  ...
  async initAUT () {
    ...
    // This must run after installing the apk, otherwise it would make the
  ...
  getPrefsBuilder () {
    if (_.isUndefined(this.prefsBuilder)) {
      this.prefsBuilder = new SharedPrefsBuilder();
    }
    return this.prefsBuilder;
  }
