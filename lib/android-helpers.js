...
helpers.unlockWithHelperApp = async function (adb) {
  await retryInterval(2, 2000, async () => {
  ...
  await retryInterval(10, 3000, async () => {
    ...
    // The command takes too much time so we should not call the command over twice in the same method.
