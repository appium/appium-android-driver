...
helpers.parseReboot = function (driverArgs) {
  if (_.isEmpty(driverArgs)) {
    return false;
  }
  if (!_.has(driverArgs, 'reboot')) {
    return false;
  }

  const reboot = driverArgs.reboot;

  if (!_.isBoolean(reboot)) {
    throw new Error(`value of key 'reboot' is not an boolean`);
  }
  return reboot;
};

helpers.parseSuppressKillServer = function (driverArgs) {
  if (_.isEmpty(driverArgs)) {
    return false;
  }
  if (!_.has(driverArgs, 'suppress-adb-kill-server')) {
    return false;
  }

  const suppressKillServer = driverArgs['suppress-adb-kill-server'];

  if (!_.isBoolean(suppressKillServer)) {
    throw new Error(`value of key 'suppress-adb-kill-server' is not an boolean`);
  }
  return suppressKillServer;
};
