 ...
 * @param {[string]} appPackages An array of package names to uninstall. If this includes `'*'`, uninstall all of 3rd party apps
 * @param {ADB} adb Instance of Appium ADB object
 * @param {[string]} filterPackages An array of packages filter with when `*` is provided
 */
helpers.uninstallOtherPackages = async function (appPackages, adb, filterPackages = []) {
  if (_.intersection(appPackages, ['*']).length === 1) {
    logger.debug('Uninstall 3rd party packages');
    appPackages = await this._get3rdPartyPackages(adb, filterPackages);
  }

  logger.debug(`Uninstalling packages: ${appPackages}`);
  await B.all(appPackages.map((appPackage) => {
    return adb.uninstallApk(appPackage);
  }));
};

helpers._get3rdPartyPackages = async function (adb, filterPackages = []) {
  try {
    const packages = await adb.shell(['pm', 'list', 'packages', '-3']);
    const appPackages = packages.trim().replace(/package:/g, '').split('\n');
    logger.debug(`'${appPackages}' filtered with '${filterPackages}'`);
    return _.difference(appPackages, filterPackages);
  } catch (err) {
