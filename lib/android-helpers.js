...
helpers.resetMockLocation = async function resetMockLocation (adb) {
  try {
    ...
    const pkgIds = await adb.fileExists(MOCK_APP_IDS_STORE)
      ? JSON.parse(await adb.shell(['cat', MOCK_APP_IDS_STORE]))
      : [];
    if (_.isEmpty(pkgIds) || (_.length(pkgIds) === 1 && pkgIds[0] === SETTINGS_HELPER_PKG_ID)) {
      await adb.shell(['appops', 'set', SETTINGS_HELPER_PKG_ID, 'android:mock_location', 'deny']);
      return;
    }

    logger.debug(`Resetting mock_location permission for the following app(s): ${pkgIds}`);
    await B.all(pkgIds.map((pkgId) => (async () => {
      try {
