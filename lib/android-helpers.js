...
helpers.unlockWithHelperApp = async function (adb) {
  try {
    ...
    logger.warn(`An error in unlockWithHelperApp: ${e}`);
  }
  ...
  await retry(3, async function () {
    ...
    if (count !== 0) {
      try {
        ...
        logger.warn(`Restart adb since "adb shell dumpsys window" commands timeout in isScreenLocked: ${e}`);
...
helpers.unlock = async function (driver, adb, capabilities) {
  ...
  if (_.isUndefined(capabilities.unlockType)) {
    ...
    logger.warn("after unlockWithHelperApp");
    // Skip helpers.verifyUnlock(adb); since the logic is included in `unlockWithHelperApp`
  } else {
