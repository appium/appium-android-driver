...
import { default as unlocker, PIN_UNLOCK, PASSWORD_UNLOCK, PATTERN_UNLOCK } from './unlock-helpers';
...
helpers.unlockWithUIAutomation = async function (driver, adb, unlock_capabilities) {
  let unlockType = unlock_capabilities.unlockType;
  if (!unlocker.isValidUnlockType(unlockType)) {
    throw new Error(`Invalid unlock type ${unlockType}`);
  }
  let unlockKey = unlock_capabilities.unlockKey;
  ...
  await unlockMethod(adb, driver, unlock_capabilities);
};
...
helpers.initDevice = async function (adb, opts) {
  ...
  if (typeof opts.unlockType === 'undefined') {
