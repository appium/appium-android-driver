...
helpers.unlock2 = async function (driver, adb, capabilities) {
  let unlockType = capabilities.unlockType;
  if (!unlocker.isValidUnlockType(unlockType)) {
    throw new Error(`Invalid unlock type ${unlockType}`);
  }
  let unlockKey = capabilities.unlockKey;
  if (! unlocker.isValidKey(unlockType, unlockKey)) {
    throw new Error(`Invalid unlock key ${unlockKey} for unlock type ${unlockType}`);
  }
  let unlockMethod;
  if (unlockType === unlocker.PIN_UNLOCK) {
    unlockMethod = unlocker.pinUnlock;
  } else if (unlockType === unlocker.PASSWORD_UNLOCK) {
    unlockMethod = unlocker.passwordUnlock;
  } else if (unlockType === unlocker.PATTERN_UNLOCK) {
    unlockMethod = unlocker.patternUnlock;
  }
  await unlockMethod(adb, driver, capabilities);
};

helpers.deprecatedUnlock = async function(adb) {
...
helpers.unlock = async function (driver, adb, capabilities) {
  ...
  if (await adb.getApiLevel() > 21) {
    await unlocker.dismissKeyguard(driver, adb);
  }
  await retryInterval(10, 1000, async () => {
    logger.debug("Screen is locked, trying to unlock");
    // check if it worked, twice
    if (capabilities.unlockType === undefined) {
      // Leave the old unlock to avoid breaking existingg tests
      logger.info("Using app unlock, this is going to be deprecated!");
      await helpers.deprecatedUnlock(adb);
    } else {
      await helpers.unlock2(driver, adb, capabilities);
    }
