...
const MOCK_LOCATION_APPS = new LRU({
  max: 100,
  updateAgeOnGet: true,
});
...
helpers.setMockLocationApp = async function setMockLocationApp (adb, app) {
  try {
    ...
    MOCK_LOCATION_APPS.set(`${adb.curDeviceId}:${app}`, app);
  } catch (err) {
...
helpers.resetMockLocation = async function resetMockLocation (adb) {
  try {
    if (await adb.getApiLevel() < 23) {
      ...
      const pkgIds = Array.of(MOCK_LOCATION_APPS.keys())
        .filter((key) => key.startsWith(`${adb.curDeviceId}:`))
        .map((key) => MOCK_LOCATION_APPS.get(key));
      if (!_.isEmpty(pkgIds)) {
        logger.debug(`Resetting mock_location permission for the following apps: ${pkgIds}`);
