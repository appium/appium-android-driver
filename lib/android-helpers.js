...
helpers.prepareEmulator = async function (adb, opts) {
  ...
  const shouldWipeData = _.includes(avdArgs, '-wipe-data');
  if (shouldWipeData && mitmCertificate) {
    logger.warn(`The provided mitmCertificate capability will be effectively ignored, ` +
      `because arguments string contains '-wipe-data' option`);
  }
  ...
  if (runningAVD) {
    if (shouldWipeData) {
      logger.debug('Killing AVD in order to wipe its data');
      await adb.killEmulator(avdName);
    } else if (mitmCertificate) {
      if (await adb.isMitmCertificateInstalled(mitmCertificate)) {
        logger.debug('Skipping root certificate installation, since it is already installed');
        return;
      }
      logger.debug('Killing AVD before installing the new root certificate');
      await adb.killEmulator(avdName);
    } else {
  ...
  if (mitmCertificate && !shouldWipeData) {
    ...
    try {
      await adb.installMitmCertificate(mitmCertificate);
    } finally {
      if (isRoot) {
        await adb.unroot();
      }
    ...
    logger.info(`Restarting AVD '${avd}' in order to finalize root certificate deployment`);
    await adb.killEmulator(avdName);
  }
