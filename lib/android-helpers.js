...
helpers.prepareEmulator = async function (adb, opts) {
  ...
  /* TODO - update appium-adb version to latest once https://github.com/appium/appium-adb/pull/245
  is merged adn published, then finish the network speed implementation
  if (_.isUndefined(opts.networkSpeed)) {
    let networkSpeed = this.ensureNetworkSpeed(adb, opts.networkSpeed);
    avdArgs = _.concat(avdArgs, ['-netspeed', networkSpeed]);
  }
  */
