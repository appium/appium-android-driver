...
helpers.resetMockLocation = async function resetMockLocation (adb) {
  try {
    if (await adb.getApiLevel() < 23) {
      ...
      const pkgIds = await helpers.getThirdPartyPackages(adb);
      if (!_.isEmpty(pkgIds)) {
        await B.all(pkgIds.map((pkgId) => (async () => {
          try {
            await adb.shell(['appops', 'set', pkgId, 'android:mock_location', 'default']);
          } catch (ign) {}
        })()));
      }
    }
...
helpers.initDevice = async function initDevice (adb, opts) {
  ...
  if (!helpers.isEmulator(adb, opts)) {
    if (mockLocationApp || _.isUndefined(mockLocationApp)) {
      ...
      logger.info('Resetting mock_location permission for all third-party apps');
