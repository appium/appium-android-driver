...
helpers.unlock = async function unlock (driver, adb, capabilities) {
  ...
  if (unlockKey && await adb.isLockManagementSupported()) {
    await unlocker.fastUnlock(unlockKey, unlockSuccessTimeout);
  } else if (!unlockType) {
    logger.warn('Using app unlock, this is going to be deprecated!');
    await helpers.unlockWithHelperApp(adb);
  } else {
    await helpers.unlockWithUIAutomation(driver, adb, {unlockType, unlockKey});
